                ...
                <div class="flex flex-col md:flex-row gap-6">
                    ...
                    <div class="flex-shrink-0">
                        @if($evento->url_imagen)
                            @php
                                $imagenUrl = str_starts_with($evento->url_imagen, 'http')
                                    ? $evento->url_imagen
                                    : asset('storage/' . $evento->url_imagen);
                            @endphp
                            <img src="{{ $imagenUrl }}" alt="{{ $evento->nombre }}" class="w-32 h-32 rounded-xl object-cover shadow-md">
                        @else
                    ...
                    <div class="flex-1">
                        ...
                        @if($evento->estado === 'finalizado' && $user_team)
                        <div class="mt-4">
                        ...
                        @if($user_team == null && !$user_is_admin && $evento->estado === 'pendiente' && auth()->user()->hasrole('Participante'))
                        <div class="mt-4 p-4 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 rounded-lg border border-purple-200 dark:border-purple-800">
                        ...
                        @elseif($user_team == null && !$user_is_admin && $evento->estado !== 'pendiente' && $evento->estado !== 'finalizado' && auth()->user()->hasrole('Participante'))
                        <div class="mt-4 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    <p class="text-sm font-semibold text-amber-800 dark:text-amber-200 mb-1">No se pueden crear equipos nuevos</p>
                                    <p class="text-sm text-amber-700 dark:text-amber-300">El evento ya ha iniciado. Solo se pueden crear equipos cuando el evento está en estado "pendiente".</p>
                                </div>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="flex flex-col md:flex-row gap-6">
                            ...
                            <a href="{{ route('equipos.show', $team) }}" class="flex-shrink-0">
                                @if($team->url_banner)
                                    ...
                                    @endphp
                                    <img src="{{ $bannerUrl }}" alt="{{ $team->nombre }}" class="w-28 h-28 rounded-xl object-cover group-hover:scale-105 transition-transform duration-200">
                                @else
